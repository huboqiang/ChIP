...
def main():
    ...
    samp_mapping.run_QC()
    samp_mapping.run_bwa()
    samp_mapping.bam2repbed(ext_len=300)
    samp_mapping.mrgbed()
    samp_mapping.mrgbed_multi()
    
    samp_peak = m02.Macs2Peaks(sam_file_chip, ref, is_debug = 0)
    samp_peak.get_shift_size_rep()
    samp_peak.get_shift_size_mrg()
    samp_peak.run_macs_rep(pvalue=0.001, ref=M_species[ref])
    samp_peak.run_macs_mrg(pvalue=0.001, ref=M_species[ref])
    samp_peak.prepare_idr_input(top_peak = 100000)
    samp_peak.run_idr()
    samp_peak.get_idr_stat()
    samp_peak.get_idr_Peak()
