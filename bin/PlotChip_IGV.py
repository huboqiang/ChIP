...
class FetchRegion(object):
    def __init__(self,l_bdg_file,region):
        ...
        self.parse_region()
#        self.__search_pos_bdg()
        self.__search_pos_bam()
...
#        print self.region
    ...
    def __search_pos_bam(self):
        self.pd_frame = {}
        self.junc     = {}
        for j, bam_file in enumerate(self.l_bdg_file):
            f_bam  = pysam.Samfile(bam_file,'rb')
            record = f_bam.fetch(reference=self.chrom, start=self.beg, end=self.end)
            l_pos  = []
            
            l_cons = np.zeros(self.end-self.beg, dtype="int")
            l_pos  = np.arange(self.end-self.beg, dtype="int") + self.beg
            M_junc = {}
            
            l_xticks = [ self.beg+1 ]
            
            bin_size = int( (self.end-self.beg)/10 )
            bin_size = 10**int(np.log10(bin_size))*10
            ...
            for rec in record:
                if rec.qname in M_qname:
                    continue
                
                M_qname[rec.qname] = 1
                chrom  = f_bam.header['SQ'][rec.rname]['SN']
                beg   = rec.reference_start
                cigar = rec.cigar
                idx   = beg - self.beg
                for pair in cigar:
                    ctag = pair[0]
...
#                    print rec, pair, beg, idx                   
                    if   ctag == 0:
                        for i in xrange(leng):
#                            print rec,pos, idx
                            if self.__is_intersect(idx+self.beg):
#                                print rec, pair,idx+pos, idx
                                l_cons[idx] += 1
                            idx += 1
                        ...
                        junc_beg = idx + self.beg
                        junc_end = idx + self.beg + leng
                        junc_pos = "%s:%d-%d" % (chrom, junc_beg, junc_end)
                        
                        if junc_pos not in M_junc:
                            M_junc[junc_pos] = {'dep':0, 'pos_l':junc_beg-self.beg, 'pos_r':junc_end-self.beg}

                        M_junc[junc_pos]['dep'] += 1
                        for i in xrange(leng):
                            idx += 1
            
            l_xticks.append( self.end )
            data = { 'pos':l_pos, 'con':l_cons }
            self.pd_frame[ bam_file ] = pd.DataFrame( data )
            self.junc[ bam_file ]     = M_junc
            if j == 0:
                print l_xticks
                self.l_xpos   = [ tick-l_xticks[0] for tick in l_xticks ]
                self.l_xticks = [ str(tick) for tick in l_xticks ]
...
class SvgIGV(FetchRegion):
    def __init__(self, out_file, l_bdg_file, region):
        ...
        self.bottom_edge_pxi = 100 # 30 for gene track + 70 for blank      
    ...
    def draw_box(self):
        ...
        self.dwg.viewbox( width=self.figure_width+100,height=self.figure_height+100 )
...
#            print " ".join(l_str_path)
    ...
    def plot_Junc(self,y_lab_max=0):
        minus_idx = self.beg+1
        total_len = self.end-self.beg
        total_sam = len(self.l_bdg_file)
        ...
        for i,bdg_file in enumerate(self.l_bdg_file):
            ...
            prefix = bdg_file.split("/")[-2]
            print prefix
            
            pos_x = self.left_edge_pxi + self.frame_width/2
            pos_y = i*(self.frame_height/total_sam)+self.top_edge_pxi+13
            
            self.dwg.add( self.dwg.text( "%s" % (prefix), ( pos_x,pos_y ),fill="black",font_size="10",text_anchor="middle" ) )
...
#            print " ".join(l_str_path)
            ...
            M_junc = self.junc[bdg_file]
            for j, junc_pos in enumerate(M_junc):
                junc_cnt   = M_junc[junc_pos]['dep']
                junc_pos_l = M_junc[junc_pos]['pos_l']
                junc_pos_r = M_junc[junc_pos]['pos_r']

                if junc_pos_l  < 0:
                    junc_pos_l = 0
                if junc_pos_r  > self.end-self.beg:
                    junc_pos_r = self.end-self.beg
                

                if y_lab_max == 0:
                    y_lab_max_used = max( self.pd_frame[ bdg_file ]['con'] )
                    y_lab_max_used = int( (y_lab_max_used)*0.3 )
                
                junc_pos_l_x = (junc_pos_l)/total_len*self.frame_width + self.left_edge_pxi
                junc_pos_r_x = (junc_pos_r)/total_len*self.frame_width + self.left_edge_pxi
                
                print M_junc[junc_pos]
                print junc_pos_l, junc_pos_r
                print junc_pos_l_x,junc_pos_r_x
                                
                y_idx = j/max(self.pd_frame[bdg_file]['con']) * len(M_junc)
                pos_y = (i+1-j*0.5/y_lab_max_used)*(self.frame_height/total_sam)+self.top_edge_pxi-20
                
                self.dwg.add( self.dwg.line( start=(junc_pos_l_x,pos_y),end=(junc_pos_r_x,pos_y),stroke="red" ) )
                self.dwg.add( self.dwg.text( "%d,%s" % (junc_cnt, junc_pos), ( (junc_pos_r_x+junc_pos_l_x)/3,pos_y-10 ),fill="red" ) )
                
            
    
    def plot_tick(self):
        tick_y_pos  = self.top_edge_pxi + self.frame_height
        tick_length = 8
        total_len = self.end-self.beg
        l_pos = [pos/total_len*self.frame_width + self.left_edge_pxi for pos in self.l_xpos]
        for i, pos in enumerate(l_pos):
            label = self.l_xticks[i]
            self.dwg.add(self.dwg.line(start=(pos, tick_y_pos), end=(pos, tick_y_pos+tick_length)))
            u = self.dwg.text("%s" % (label), (pos, tick_y_pos+tick_length*2),fill="black" )
            u.rotate(30,center=(pos, tick_y_pos+tick_length*2))
            self.dwg.add(u)
