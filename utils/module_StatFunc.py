    ...
    def read_infile(self):
        if os.path.isfile( self.infile ):
            ...
            if len( f_1[5].split(';') ) > 1:
                self.ER_l_rate  = float(f_1[5].split(';')[0])
                self.ER_r_rate  = float(f_1[5].split(';')[1])
                self.Q20_l_rate = float(f_1[6].split(';')[0])
                self.Q20_r_rate = float(f_1[6].split(';')[1])
                self.Q30_l_rate = float(f_1[7].split(';')[0])
                self.Q30_r_rate = float(f_1[7].split(';')[1])
                self.GC_l_rate  = float(f_1[8].split(';')[0])
                self.GC_r_rate  = float(f_1[8].split(';')[1])
            else:
                self.ER_l_rate  = float(f_1[5].split(';')[0])
                self.Q20_l_rate = float(f_1[6].split(';')[0])
                self.Q30_l_rate = float(f_1[7].split(';')[0])
...
class BwaPicard(object):
    ...
    def read_q30_bed( self,bed_q30 ):
        self.q30    = self.__get_line_cnt( bed_q30  )
   
    def __get_line_cnt( self,infile_bed ):
        lincnt = 0
        if os.path.isfile(infile_bed):
            is_gzipped = check_gzipped( infile_bed )
            shell_work = 'wc -l %s' % ( infile_bed )
            if is_gzipped:
                shell_work = 'zcat %s | wc -l ' % ( infile_bed )
